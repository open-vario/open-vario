...
bool UdpSocket::close()
{
    ...
    if (m_socket != INVALID_SOCKET)
    {
        int callRet = closesocket(m_socket);
        if (callRet == 0)
        {
            m_socket = INVALID_SOCKET;
            ret = true;
...
bool UdpSocket::bindAddr(const IpEndPoint& local_address)
{
    ...
    if (m_socket != INVALID_SOCKET)
    {
        ...
        const int callRet = ::bind(m_socket, reinterpret_cast<struct sockaddr *>(&addr), sizeof(addr));
        if (callRet == 0)
        {
            ret = true;
        }
...
bool UdpSocket::receiveFrom(void* data, const size_t size, size_t& received, IpEndPoint& distant_address, const uint32_t timeout)
{
        ...
        (size != 0))
    {
        ...
        int callRet = select(1, &readfds, NULL, NULL, &tv);
        if( callRet > 0)
        {
            struct sockaddr_in remoteAddr;
            int addrLen = sizeof(remoteAddr);

            callRet = ::recvfrom(m_socket, reinterpret_cast<char *>(data), static_cast<int>(size), 0, reinterpret_cast<struct sockaddr *>(&remoteAddr), &addrLen);
            if (callRet > 0)
            {
                // Data has been received
                received = static_cast<uint32_t>(callRet);
...
bool UdpSocket::sendTo(const void* data, const size_t size, const IpEndPoint& distant_address)
{
        ...
        (size != 0))
    {
        ...
        int callRet = ::sendto(m_socket, reinterpret_cast<const char *>(data), static_cast<int>(size), 0, reinterpret_cast<struct sockaddr *>(&remoteAddr), sizeof(struct sockaddr_in));
        if (callRet == static_cast<int>(size))
        {
            // Data sent
            ret = true;
        }
