...
, m_first_free_sector(FileHeader::NO_SECTOR)
, m_read_context()
, m_write_context()
{}
...
bool NorFlashFs::init()
{
    ...
    if (ret)
    {
        ...
        if (sector_valid &&
            (sector_header.sector_type == NorFlashSectorHeader::START_OF_FS))
        {
            // Read file system header
            StartFsHeader start_fs_header;
            uint32_t current_address = toAddress(0) + sizeof(NorFlashSectorHeader);
            ...
            if (ret)
            {
                ...
                if (start_fs_header.version == NORFLASHFS_VERSION)
                {
                    // Read file table
                    FileEntry entry = {0};
                    FileEntry last_entry = {0};
                    do
                    {
                        // Read entry
                        ret = m_nor_flash.read(current_address, &entry, sizeof(FileEntry));
                        if (ret)
                        {
                            // Check if entry is empty
                            if (entry.file_start_sector != FileEntry::EMPTY)
                            {
                                m_file_count++;
                                m_current_file_id++;
                                last_entry = entry;
                    ...
                    if (ret)
                    {
                        ...
                        else
                        {
                            ...
                            ret = readSectorHeader(last_entry.file_start_sector, sector_header, sector_valid);
                            if (ret && (sector_header.sector_type == NorFlashSectorHeader::START_OF_FILE))
                            {
                                current_address = toAddress(last_entry.file_start_sector) + sizeof(NorFlashSectorHeader);
                                ret = m_nor_flash.read(current_address, &m_write_context.file_header, sizeof(FileHeader)); 
                                if (ret)
                                {
                                    // Check if the file has been finalized
                                    if (m_write_context.file_header.size == FileHeader::NO_SIZE)
                                    {
                                        // Look for end of file
                                        uint32_t end_sector = 0;
                                        uint8_t end_sector_type = 0;
                                        ret = goToEndOfFileByDataSector(last_entry.file_start_sector, end_sector, end_sector_type);
                                        if (ret)
                                        {
                                            // Finalize file
                                            if (end_sector_type == NorFlashSectorHeader::RESERVED)
                                            {
                                                end_sector--;
                                            }
...
bool NorFlashFs::format()
{
    ...
    if (ret)
    {
        ...
        if (ret)
        {
            ...
            start_fs_header.reserved = StartFsHeader::RESERVED;
...
bool NorFlashFs::createFile(const char name[])
{
    ...
    if (!m_write_context.is_in_use)
    {
        ...
        m_write_context.file_sector = m_first_free_sector;
        m_write_context.current_sector = m_first_free_sector;
        ...
        if (ret)
        {
            ...
            file_header.last_sector = StartFsHeader::RESERVED;
            ...
            ret = write(&file_header, sizeof(FileHeader));
            if (ret)
            {
                // Update file header
                file_header.size = 0;
...
bool NorFlashFs::closeWrittenFile()
{
    ...
    {
        // Compute next sector number
        m_first_free_sector = m_write_context.current_sector + 1;
            ...
            if (ret)
            {
                ...
                if (sector_type == NorFlashSectorHeader::START_OF_FILE)
                {
                    ...
                    ret = m_nor_flash.write(current_address + sizeof(FileDataHeader::id), 
                                            ...
                                            sizeof(FileHeader::size) + sizeof(FileHeader::last_sector));
                    if (ret)
                    {
                        // File closed
...
bool NorFlashFs::writeSectorHeader(const uint32_t sector, const uint8_t sector_type)
{
    ...
    // Write sector header
    m_write_context.current_address = toAddress(sector); 
    const bool ret = write(&header, sizeof(NorFlashSectorHeader));
    return ret;
}
...
bool NorFlashFs::goToEndOfFileByDataSector(const uint32_t current_sector, uint32_t& end_sector, uint8_t& end_sector_type)
{
    ...
    do
    {
        ...
        bool valid = false;
        NorFlashSectorHeader sector_header;
        ret = readSectorHeader(m_write_context.current_sector, sector_header, valid);
        if (ret)
        {
            // Check sector type
            uint8_t sector_type = sector_header.sector_type; 
            m_write_context.current_address = toAddress(m_write_context.current_sector) + sizeof(NorFlashSectorHeader);
            if ((sector_type == NorFlashSectorHeader::FILE_DATA) ||
                ((m_write_context.current_sector == current_sector) && (sector_type == NorFlashSectorHeader::START_OF_FILE)))
            {
                // Data sector or start of file sector => still in current file
                if (sector_type == NorFlashSectorHeader::START_OF_FILE)
                {
    ...
    if (ret)
    {
        ...
        if (ret)
        {
            ...
            ret = m_nor_flash.write(current_address + sizeof(FileDataHeader::id) + sizeof(FileDataHeader::file_start_sector), 
                                    ...
                                    sizeof(FileDataHeader::data_size));
        }
    }
...
bool NorFlashFs::write(const void* data, const size_t size)
{
    const bool ret = m_nor_flash.write(m_write_context.current_address, data, size);
    if (ret)
    {
