...
, m_config_filter_depth(OV_CONFIG_VALUE_ALTI_FILTER_DEPTH, "Raw value filter depth", 5u, 1u, m_alti_filter.getMaxFilterDepth(), false)
...
bool Altimeter::init()
{
    ...
    ret = ret && m_config_manager.getConfigValue<uint8_t>(OV_CONFIG_GROUP_ALTIMETER, OV_CONFIG_VALUE_ALTI_FILTER_DEPTH, filter_depth);
    ...
    if (ret)
    {
        ...
        m_alti_filter.setFilterDepth(filter_depth);

        // Initialize min and max values
        m_alti_values.min_alti = 2147483647;
        m_alti_values.max_alti = -2147483648;
    }
...
bool Altimeter::compute(int32_t& raw_altitude, uint32_t& raw_pressure, int16_t& raw_temperature)
{
    ...
    if (ret)
    {
        ...
        m_alti_values.main_alti = m_alti_filter.compute(raw_altitude);
        ...
        if (m_alti_values.main_alti < m_alti_values.min_alti)
        {
            m_alti_values.min_alti = m_alti_values.main_alti;
        }
        if (m_alti_values.main_alti > m_alti_values.max_alti)
        {
            m_alti_values.max_alti = m_alti_values.main_alti;
        }
